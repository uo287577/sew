...
class Fondo
{
    ...
    getImagenFondo()
    {
        var flickrAPI = "http://api.flickr.com/services/feeds/photos_public.gne?jsoncallback=?"; // NO CONSEGUIMOS QUE FUNCIONE CON COORDENADAS POR EMA PERMISOS O A SABER
        ...
        .done(function(data) {
            $.each(data.items, function(i,item ) {
                $("<img />").attr( "src", item.media.m).attr("alt", "Imagen de la F1 de fondo de la pantalla principal")
                .css({
                    "width": "100%",
                    "height": "auto"})
                .appendTo( "body > main" );
